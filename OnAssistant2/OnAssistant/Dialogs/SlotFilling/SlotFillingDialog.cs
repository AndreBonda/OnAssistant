            ...
            switch (slotName)
            {
                case BotNames.CodiceFiscale:
                    ...
                    if (!string.IsNullOrEmpty(paw))
                    {
                        ...
                        // Richiede stesso inserimento se paw non trovato
                        return await dialogContext.BeginDialogAsync(BotNames.TextCF, new PromptOptions { Prompt = MessageFactory.Text("Non è stato trovato nessun paziente web collegato al tuo codice fiscale. Reinserisci il tuo codice fiscale") }, cancellationToken);
                    }
